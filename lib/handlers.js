...
var debug = util.debug("performance");
      ...
      if (!err && str) {
        ...
        helpers.addUniversalTemplates(str, templateData, function (err, str) {
          if (!err && str) {
            ...
            callback(
              500,
              undefined + " Error: @addUniversalTemplate, msg:" + err,
              "html"
            );
          }
        });
      } else {
        callback(500, undefined + " Error: @index, msg:" + err + str, "html");
      }
    });
  } else {
  ...
  if (data.method == "get") {
    ...
    helpers.getTemplate("accountCreate", templateData, function (err, str) {
      if (!err && str) {
        ...
        callback(
          500,
          undefined + " Error: @accountCreate, msg:" + err + str,
          "html"
        );
      }
    });
  ...
  if (data.method == "get") {
    ...
    helpers.getTemplate("sessionCreate", templateData, function (err, str) {
      if (!err && str) {
        ...
        callback(
          500,
          undefined + " Error: @sessionCreate, msg:" + err + str,
          "html"
        );
      }
    });
  ...
  if (data.method == "get") {
    ...
    helpers.getTemplate("sessionDeleted", templateData, function (err, str) {
      if (!err && str) {
        ...
        callback(
          500,
          undefined + " Error: @sessionDeleted, msg:" + err + str,
          "html"
        );
      }
    });
  ...
  if (data.method == "get") {
    ...
    helpers.getTemplate("accountEdit", templateData, function (err, str) {
      if (!err && str) {
        ...
        callback(
          500,
          undefined + " Error: @accountEdit, msg:" + err + str,
          "html"
        );
      }
    });
  ...
  if (data.method == "get") {
    ...
    helpers.getTemplate("accountDeleted", templateData, function (err, str) {
      if (!err && str) {
        ...
        callback(
          500,
          undefined + " Error: @accountDeleted, msg:" + err + str,
          "html"
        );
      }
    });
  ...
  if (data.method == "get") {
    ...
    helpers.getTemplate("checksCreate", templateData, function (err, str) {
      if (!err && str) {
        ...
        callback(
          500,
          undefined + " Error: @checksCreate, msg:" + err + str,
          "html"
        );
      }
    });
  ...
  if (data.method == "get") {
    ...
    helpers.getTemplate("checksList", templateData, function (err, str) {
      if (!err && str) {
        ...
        callback(
          500,
          undefined + " Error: @checksList, msg:" + err + str,
          "html"
        );
      }
    });
  ...
  if (data.method == "get") {
    ...
    helpers.getTemplate("checksEdit", templateData, function (err, str) {
      if (!err && str) {
        ...
        callback(
          500,
          undefined + " Error: @checksEdit, msg:" + err + str,
          "html"
        );
      }
    });
  ...
  if (data.method == "get") {
    ...
    if (trimmedAssetName.length > 0) {
      ...
      helpers.getStaticAsset(trimmedAssetName, function (err, data) {
        if (!err && data) {
          // Determine the contetn type (default to plain text)
                    ...
                    _data.delete("checks", checkId, function (err) {
                      ...
                      if (checksDeleted == checksToDelete) {
                        if (!deletionErrors) {
                          ...
                          callback(500, {
                            Error:
                              "Errors encountered while attempting to delete all of the the user's checks. all checks may not have been delete from the system successfully",
                          });
                        }
                      }
                    });
...
handlers._tokens.post = function (data, callback) {
  ...
  if (phone && password) {
    ...
    _data.read("users", phone, function (err, userData) {
      ...
      if (!err && userData) {
        // Hash the sent password, and compare it to the password stored in users object
      ...
      if (!err && tokenData) {
        ...
        _data.read("users", userPhone, function (err, userData) {
          if (!err && userData) {
            ...
            // Verify that the user has less than the number of max-checsks-per-user
            if (userChecks.length < config.maxChecks) {
              // Veriy that the URL given has DNS entries (and therefore can resolve)
              ...
              dns.resolve(hostName, function (err, records) {
                if (!err && records) {
                  ...
                  // Create the check objec, and include the user's phone
                  var checkObject = {
                    id: checkId,
                    userPhone: userPhone,
                    protocol: protocol,
    ...
    if (protocol || url || method || successCodes || timeoutSeconds) {
      ...
      _data.read("checks", id, function (err, checkData) {
        if (!err && checkData) {
          ...
          // Verify that the given token is valid and belongs to the user who create the checks
          handlers._tokens.verifyToken(
            token,
            checkData.userPhone,
            function (tokenIsValid) {
              if (tokenIsValid) {
                ...
                _data.update("checks", id, checkData, function (err) {
                  if (!err) {
                    ...
                    callback(403, { Error: "Could not update the check" });
                  }
                });
              } else {
                callback(403);
                  ...
                  _data.read(
                    ...
                    function (err, userData) {
                      if (!err && userData) {
                        ...
                        callback(400, {
                          Error:
                            "Could not find the user who created the check, so could not remove the check from the list of user object",
                        });
                      }
                    }
                  );
